...
const MyDocuments = () => {
  ...
  useEffect(() => {
    ...
    socket.onmessage = (event) => {
      ...
      if (data.action === 'document_displayed') {
        // const value = data.documents
        // setMyDocuments({...mydocuments,value});
        console.log(data.documents,"lllllllllllllllllllllllllllllllllllllllllllll");
      }
    };
  ...
  return (
    <div>
      ...
      <button onClick={() => editDocument({ /* document data */ })}>
        Edit Document
      </button>
      ...
      sx={{ width: '100%' }}
    >
    {mydocuments&& mydocuments.map((item) => (
      <Grid xs={6}>
        <Item>
          <div className='d-flex flex-row justify-content-between '>
            <div className='notes-text'>{item.title}
            <br />hii
            {item.content}</div>
      ...
      {showAddModal && <AddDocumentsModal setShowAddModal={setShowAddModal}/>}
    </div>
  );
};
